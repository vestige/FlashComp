...
- [x] `EditEvent` の設定進捗を可視化（シーズン/カテゴリ/課題）
- [x] `EditEvent` / `EventClimbers` / `EventScores` のクイックナビを共通化
- [x] イベント編集画面でシーズン追加をポップアップで行えるようにする（Create Eventと同系UI）
- [x] イベント編集画面でカテゴリ追加をポップアップで行えるようにする（Create Eventと同系UI）

### 優先3: シーズン単位の編集導線
- [x] シーズン一覧をカードUI化（Eventカード風のステータス表示）
- [x] シーズン編集画面を作る
- [x] 課題の追加はシーズン編集画面で行う
...
### Bug
- [x] イベント設定で指定した期間ではないシーズンが追加できてしまう
