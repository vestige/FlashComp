...
const sampleGymIdsByEmail = {
  "owner.shibuya@example.com": ["gym-shibuya"],
  "owner.yokohama@example.com": ["gym-yokohama"],
  "owner.multi@example.com": ["gym-shibuya", "gym-yokohama"],
};
...
async function run() {
  ...
  if (!profileSnap.exists()) {
    ...
    if (byEmail.empty) {
      ...
      const fallbackGymIds = sampleGymIdsByEmail[ownerEmail] || [];
      const bootstrapGymIds = envGymIds.length > 0 ? envGymIds : fallbackGymIds;

      if (bootstrapGymIds.length === 0) {
      ...
      const gymIds = Array.isArray(profile.gymIds) ? profile.gymIds : [];
      if (!gymIds.length) {
        console.log("[fail] gymIds is empty. Owner cannot manage events.");
        process.exit(1);
      }

      const allowedGymId = explicitAllowedGymId || gymIds[0];
